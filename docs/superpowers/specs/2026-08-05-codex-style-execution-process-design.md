...
前端根据真实顺序进行纯展示分组，不推断业务阶段：

- reasoning summary 或中间 agent message 是过程说明。
- 紧随其后的连续工具 Item 归入该说明对应的步骤组。
- 下一个过程说明出现时开始新的步骤组。
- 没有前置说明的工具 Item 进入无标题工具组。
...
- 过程说明及其后续连续工具 Item 组成可折叠步骤组；分组只依据事件相邻关系，不猜测阶段含义。
- 步骤组默认折叠；正在运行的当前步骤组自动展开。
...
- summary 与其后连续工具 Item 组成稳定的嵌套步骤组。
