...
| 分析工作台 | 左侧对话、过程和追问；右侧持续生成、修改当前分析结果 | 不展示团队共享资产库 |
| 我的分析 | 查看已保存结果、历史版本和分析模板，并回到分析工作台继续 | 不做当前任务里的草稿编辑主界面 |
...
- 分析结果为中心：分析任务是过程；确认后的结果可作为内部 `Artifact` 被保存、治理和复用。
- 右侧结果优先：聊天是协作过程，交互式报告才是用户要保存、分享、导出与复用的交付物。
