...
from backend.analysis.agent_runner import AnalysisAgentRunResult, AnalysisLLMProviderConfig, build_analysis_runner_prompt
from backend.analysis.run_service import AnalysisRunRequest, AnalysisRunService
from backend.exploration.agent_runner import ExplorationAgentRunnerEvent, LLMProviderConfig


class AnalysisAgentRunnerTest(unittest.TestCase):
    def test_analysis_provider_config_uses_analysis_model_override(self) -> None:
        with patch.dict(
            os.environ,
            {
                "GENBI_ENV_FILE": "missing-test.env",
                "OPENAI_API_KEY": "sk-test",
                "GENBI_EXPLORATION_MODEL": "gpt-exploration",
                "GENBI_ANALYSIS_MODEL": "gpt-analysis",
            },
            clear=True,
        ):
            config = AnalysisLLMProviderConfig.from_env()

        self.assertEqual(config.provider_config.model, "gpt-analysis")
        config.validate()

    def test_analysis_provider_config_error_mentions_analysis_runtime(self) -> None:
        config = AnalysisLLMProviderConfig(
            provider_config=LLMProviderConfig(provider="openai", model="", api_key="sk-test"),
        )

        with self.assertRaisesRegex(RuntimeError, "GENBI_ANALYSIS_MODEL"):
            config.validate()
    ...
    def test_analysis_service_forwards_sync_agent_runner_events(self) -> None:
        class FakeRunner:
            def stream(self, prompt: str):
                ...
                yield AnalysisAgentRunResult(final_output="真实 SDK Runner 输出", raw_result_type="FakeResult")
        ...
        self.assertEqual(message.payload["content"], "真实 SDK Runner 输出")
    ...
    def test_analysis_service_forwards_async_agent_runner_events(self) -> None:
        class FakeRunner:
            async def async_stream(self, prompt: str):
                ...
                yield AnalysisAgentRunResult(final_output="异步 SDK Runner 输出", raw_result_type="FakeStreamedResult")
        ...
        self.assertIn("agent.runner.raw", [event.type for event in events])
        message = next(event for event in events if event.type == "agent.message.created")
        self.assertEqual(message.payload["content"], "异步 SDK Runner 输出")
